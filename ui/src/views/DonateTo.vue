  ...
  <div>
    <md-card>
      <md-card-header>
        ...
        <p></p>
        <div class="md-title">Eco-investments in your name, <strong>Mr Green</strong></div>
        <div class="md-subhead">Your activities resulted in <strong> {{ "£ " + total.toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }} </strong> of eco-investments in <strong>Oct 2019</strong></div>
        <p></p>
      </md-card-header>
      ...
      <md-card-content>
        ...
        <md-table v-model="items" md-card>
          <md-table-row slot="md-table-row" slot-scope="{ item }">
            <md-table-cell md-label="Charity Name" md-sort-by="charityName">
              {{ item.charityName }}
            </md-table-cell>
...
import {cCharitiesData} from '../data';

export default {
  name: 'DonateFrom',

  data() { return {
    items: JSON.parse(cCharitiesData).data,
    total: null
  }},
